...
export const defaultContent = {
  ...
  photos: galleryImages.map((image, index) => ({
    ...
    src: image.src,
    alt: image.alt,
    altEn: image.altEn ?? image.alt,
    altHu: image.altHu ?? image.alt,
    storagePath: null,
    sortOrder: index,
    isVisible: true,
  })),
...
export function normalizeContent(remoteRows = {}) {
  ...
  const heroSlides = (Array.isArray(rows.heroSlides) ? rows.heroSlides : []).map((row, index) => ({
    ...
    altEn: row.altTextEn ?? row.alt_text_en ?? row.altEn ?? '',
    altHu: row.altTextHu ?? row.alt_text_hu ?? row.altHu ?? '',
    photoId: row.photo_id ?? row.photoId ?? null,
    caption: row.caption ?? '',
